...
module.exports = function(logGroupName, startTime, requestId) {
  ...
  logStreamLocator._requestStreamHasBeenFound = function() {
    return !_.isEmpty(logStreamForRequestId);
  };
   ...
   * @private
   */
  logStreamLocator._logStreamFound = function(callback) {
    callback(null, logStreamForRequestId);
  };
  ...
  logStreamLocator._testForStartEvent = function(stream, event) {
    if (logFilter.isStartEvent(event)) {
      logStreamForRequestId = stream.logStreamName;
    }
  };
   ...
   * @private
   */
  logStreamLocator._searchStreamForStartEvent = function(err, data, stream, callback) {
    if (!err) {
      _.each(
        data.events,
  ...
  logStreamLocator.getLogStreamForRequestId = function(callback) {
    async.until(
      ...
      _.partial(_.delay, logStreamLocator._checkForStartEvent, 1000),
