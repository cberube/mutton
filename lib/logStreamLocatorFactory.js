...
module.exports = function(logGroupName, startTime, requestId) {
  ...
  var logStreamLocator = {
    logStreamForRequestId: undefined
  };
  var logStreams = [ ];
  ...
  logStreamLocator._requestStreamHasBeenFound = function() {
    return !_.isEmpty(logStreamLocator.logStreamForRequestId);
  };
  ...
  logStreamLocator._logStreamFound = function(callback) {
    callback(null, logStreamLocator.logStreamForRequestId);
  };
  ...
  logStreamLocator._testForStartEvent = function(stream, event) {
    if (logFilter.isStartEvent(event)) {
      logStreamLocator.logStreamForRequestId = stream.logStreamName;
    }
  };
