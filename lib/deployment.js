...
deployment._replaceVariables = function(template) {
  if (typeof(template) !== 'string') {
    return template;
  }

  return mustache.render(template, deployment.config.variables);
};
...
deployment.packageFunction = function(functionDetails, callback) {
  var zipCommand = 'zip -rq ' + functionDetails.archivePath + ' .';
...
deployment.loadConfigurationFile = function(functionDetails, callback) {
  ...
  try {
    ...
    config = _.mapValues(config, deployment._replaceVariables);
  } catch (ex) {
    callback(
...
/**
 * The callback function to invocationHelper when all of the functions currently being
